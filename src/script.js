...
const app = {
    ...
    formatWindTemp(raw) {
        ...
        let dirCode = parseInt(match[1], 10), spdCode = parseInt(match[2], 10);
        let dir, spd;
        if (dirCode === 99) { dir = "VRB"; spd = spdCode; } 
        else if (dirCode >= 50) { dir = (dirCode - 50) * 10; spd = spdCode + 100; } 
        else { dir = dirCode * 10; spd = spdCode; }
        
        let dirStr = dir === "VRB" ? "VRB" : String(dir).padStart(3, '0'), spdStr = String(spd).padStart(3, '0');
        let tempStr = (match[3] && match[4]) ? " " + (match[3] === 'M' ? '-' : '') + match[4] : "";
    ...
    calculate() {
        ...
        this.state.waypoints.forEach((wp, i) => {
            ...
            // ★ 今回の修正: そのWPTの「推測高度」に合わせて、動的に予測風・気温を算出する
    ...
    render() {
        ...
        if (!isAlreadyRendered) {
            ...
            this.state.waypoints.forEach((wp, i) => {
                ...
                // ★ 今回の修正: 推測風・推測気温が計画値から変わっていたら色を変える
