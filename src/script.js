...
const app = {
    ...
    renderForecastCard() {
        ...
        if (!card) {
            ...
            card.className = 'card-drm no-print'; // ★ DRMと同じクラスを適用
            card.style.marginBottom = '15px';     // ★ 余白を維持
            
            // ★ DRMと完全に同一のHTML構造に変更し、インラインスタイルを排除
            card.innerHTML = `
                <div class="drm-header" onclick="app.toggleForecast()">
                    <span class="drm-title">☁️ WINDS/TEMP ALOFT FCST</span>
                    <span id="fcst-toggle-icon" style="color: var(--text-faint);">▶</span>
                </div>
                <div id="forecastInfoContent" class="drm-content" style="display: none;"></div>
            `;
            // ★ DRMカードの「後（下）」に挿入
