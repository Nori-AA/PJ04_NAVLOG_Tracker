...
const app = {
    ...
    saveFlightToHistory() {
        if (!this.state.flightMeta) return;
        ...
        this.state.flightHistory.forEach((h, idx) => {
            ...
            div.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 2px;">
                    ...
                    <div style="font-size: 11px; color: var(--accent-color);">👨‍✈️ Crew: ${otherCrewNames || '(No entry)'}</div>
                </div>
                <button class="btn-secondary btn-small" onclick="app.applyCrewFromHistory(${idx})" style="padding: 5px 8px; font-size: 10px;">[このCrewを適用]</button>
            `;
    ...
    resetData() {
        if (!confirm("フライトデータをリセットしますか？")) return;

        const keepCrew = confirm("Crew情報を次の便に引き継ぎますか？\n(T/O・LDGのチェックは解除されます)");

        const oldCrew = JSON.parse(JSON.stringify(this.state.crew));
        const oldHistory = JSON.parse(JSON.stringify(this.state.flightHistory));

        this.state = { 
            waypoints: [], altns: [{name:'', fuel:0, rsv:0}], alertThreshold: 0, destFuelThreshold: 0, headerInfo: "", flightMeta: null, fuelCalcBasis: 'CALC',
            crew: keepCrew ? oldCrew : [{ id: 1, duty: 'PIC', empNo: '42482', name: 'NORIYUKI ARAI', rank: 'CAP' }, { id: 2, duty: 'COP', empNo: '', name: '', rank: 'COP' }],
            crewMemo: keepCrew ? this.state.crewMemo : '', 
            takeoffPilotId: null, 
            landingPilotId: null, 
            crewPanelOpen: true,
            times: { bo: '', bi: '', tkof: '', ldg: '' },
