...
const app = {
    ...
    calcPflTimes() {
        ...
        // DEP / ARR タイム算出
        if (bo && bo.length === 4) {
            pfl.depTime = this.convertZtoLocal(bo, pfl.domInt);
        }
        if (bi && bi.length === 4) {
    ...
    convertZtoLocal(zTime, domInt) {
        ...
        let m = this.toMin(zTime) + 9 * 60; // +9h (JST)
        m = m % 1440;
    ...
    renderPostFlightLog() {
        ...
        const btnDom = document.getElementById('btnPflDom');
        if(btnDom) btnDom.className = pfl.domInt === 'DOM' ? 'pfl-btn active' : 'pfl-btn';
        const btnInt = document.getElementById('btnPflInt');
        if(btnInt) btnInt.className = pfl.domInt === 'INT' ? 'pfl-btn active' : 'pfl-btn';
        
        const btnPic = document.getElementById('btnPflPic');
        if(btnPic) btnPic.className = pfl.activeDuty === 'PIC' ? 'pfl-btn active' : 'pfl-btn';
        const btnSic = document.getElementById('btnPflSic');
        if(btnSic) btnSic.className = pfl.activeDuty === 'SIC' ? 'pfl-btn active' : 'pfl-btn';
        const btnCop = document.getElementById('btnPflCop');
        if(btnCop) btnCop.className = pfl.activeDuty === 'COP' ? 'pfl-btn active' : 'pfl-btn';
